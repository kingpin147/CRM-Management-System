...
import { Camera, UploadCloud, RefreshCw, X, CheckCircle2, SwitchCamera, Image as ImageIcon, Eye, Trash2 } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Badge } from '@/components/ui/badge'

interface CnicCameraCaptureProps {
  label: string
  cardSide: 'front' | 'back'
  file: File | null
...
export function CnicCameraCapture({
  ...
  const startCameraStream = useCallback(async (mode: 'environment' | 'user') => {
    ...
    try {
      if (!navigator?.mediaDevices?.getUserMedia) {
        throw new Error('Camera access is not supported by your browser. Please use native camera or gallery upload.')
      }
      ...
      if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
        setCameraError('Camera permission denied. Please allow camera access in browser settings, or use the Native Camera button.')
  ...
  const handleConfirmSnapshot = () => {
    ...
    canvasRef.current.toBlob((blob) => {
      if (blob) {
        ...
        const fileName = `cnic_${cardSide}_camera_${timestamp}.jpg`
  ...
  return (
    <div className="space-y-2">
      ...
      {/* Label and Side Badge */}
      <div className="flex items-center justify-between">
        <label className="text-xs font-semibold text-slate-800 flex items-center gap-1.5">
          <span>{label}</span>
          <span className="text-[10px] font-bold text-amber-900 bg-amber-100 px-1.5 py-0.5 rounded border border-amber-300 uppercase">
            CNIC {cardSide}
          </span>
        </label>
        {file && (
      ...
      {!file ? (
        <div className="border-2 border-dashed border-slate-300 hover:border-amber-400 bg-slate-50/60 hover:bg-amber-50/20 rounded-xl p-3.5 transition-all text-center space-y-3">
          <div className="flex flex-col sm:flex-row items-center justify-center gap-2">
            ...
            <Button
              ...
              className="w-full sm:w-auto bg-[#135d86] hover:bg-[#002868] text-white text-xs font-bold gap-1.5 shadow-xs cursor-pointer h-9 px-3.5"
            >
              <Camera className="w-4 h-4 text-amber-300" />
              <span>Take Photo (Camera)</span>
            </Button>

            {/* Direct Device Native Camera (Mobile optimized fallback) */}
            <Button
              type="button"
              variant="outline"
              size="sm"
              ...
              className="w-full sm:w-auto text-xs font-semibold border-amber-300 bg-amber-50/50 hover:bg-amber-100 text-amber-900 gap-1.5 h-9 px-3 cursor-pointer"
            ...
            <Button
              ...
              className="w-full sm:w-auto text-xs font-semibold border-slate-300 hover:bg-white text-slate-700 gap-1.5 h-9 px-3 cursor-pointer"
            >
          ...
          <p className="text-[11px] text-slate-500 font-medium">
            Take a clear photo of CNIC <strong className="text-slate-700 font-bold uppercase">{cardSide}</strong> or upload from gallery.
          </p>
        </div>
      ) : (
        ...
        <div className="flex items-center gap-3 p-2.5 bg-slate-50 border border-slate-200 rounded-xl shadow-2xs">
          {/* Thumbnail Preview */}
          <div className="relative w-20 h-14 rounded-lg overflow-hidden bg-slate-900 shrink-0 border border-slate-300 group">
            {previewUrl ? (
              <img
                src={previewUrl}
                alt={`CNIC ${cardSide}`}
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-slate-500 font-mono">
              {(file.size / 1024).toFixed(1)} KB • CNIC {cardSide.toUpperCase()}
            </p>
          </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-bold text-white tracking-tight">
                    Take CNIC <span className="uppercase text-amber-400">{cardSide}</span> Photo
                  </h3>
                  <p className="text-[11px] text-slate-400">
                    Align your CNIC card inside the guidelines and snap a clear picture.
                  </p>
                </div>
              </div>
              ...
              {cameraError ? (
                ...
                <div className="relative w-full h-full flex items-center justify-center bg-black">
                  ...
                  <div className="absolute top-3 left-3 bg-black/70 backdrop-blur-xs text-amber-400 text-xs font-bold px-2.5 py-1 rounded-md border border-amber-500/30">
                    Preview Snapshot (CNIC {cardSide.toUpperCase()})
                  </div>
                </div>
              ) : (
                /* Live Camera Stream */
                  ...
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none p-6">
                    <div className="w-[85%] max-w-[440px] aspect-[1.586/1] border-2 border-dashed border-amber-400/90 rounded-2xl shadow-[0_0_0_9999px_rgba(0,0,0,0.45)] relative flex flex-col items-center justify-between p-3.5">
                      ...
                      <span className="text-[11px] font-bold text-amber-300 bg-black/70 px-2 py-0.5 rounded-full tracking-wide">
                        CNIC {cardSide.toUpperCase()} HERE
                      </span>
