...
import { Search, RotateCcw, Zap, Receipt, Users, CheckCircle2, Clock, AlertTriangle, FileSpreadsheet, ChevronDown, Cpu, Battery, Sun } from 'lucide-react'
...
function getInverterSerials(c: CustomerRecord): string[] {
  if (c.solarSystem?.inverterSerials && Array.isArray(c.solarSystem.inverterSerials) && c.solarSystem.inverterSerials.length > 0) {
    return c.solarSystem.inverterSerials.filter(Boolean)
  }
  if (c.solarSystem?.inverterSerial) {
    return c.solarSystem.inverterSerial.split(',').map((s: string) => s.trim()).filter(Boolean)
  }
  return []
}

function getBatterySerials(c: CustomerRecord): string[] {
  if (c.solarSystem?.batterySerials && Array.isArray(c.solarSystem.batterySerials) && c.solarSystem.batterySerials.length > 0) {
    return c.solarSystem.batterySerials.filter(Boolean)
  }
  if (c.solarSystem?.batterySerial) {
    return c.solarSystem.batterySerial.split(',').map((s: string) => s.trim()).filter(Boolean)
  }
  return []
}
...
// CONNECTIVITY REPORT COMPUTATION
  ...
  const filteredCustomers = React.useMemo(() => {
    ...
    return customers.filter((c) => {
      ...
      // Filters only for non-brand categories
      if (!isBrandReport) {
  ...
  }, [customers, activeCategory, hasSearched, appliedFilters, isBrandReport])
  ...
  const handleTabChange = (categoryId: string) => {
    setHasSearched(false)
    const viewSlug = categoryId.toLowerCase().replace(/_/g, '-')
    router.push(`/dashboard/reports?view=${viewSlug}`, { scroll: false })
  }
      ...
      <Card className="shadow-sm border-slate-200 overflow-hidden bg-white">
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            ...
            {!isBrandReport && activeCategory !== 'REGISTER' && (activeCategory !== 'SALES' && activeCategory !== 'CONNECTIVITY') && activeCategory !== 'RECEIVABLE' && activeCategory !== 'ADJUSTMENT' && activeCategory !== 'PAYMENTS' && activeCategory !== 'BILLING' && (
              <div className="space-y-1">
            ...
            {!isBrandReport && activeCategory !== 'RECEIVABLE' && (
              <div className="space-y-1">
