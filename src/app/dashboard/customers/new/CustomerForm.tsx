...
const customerSchema = z.object({
  ...
  // TAB 4: Installer Details & System Audit
...
export function CustomerForm({ users }: { users?: { id: string, fullName: string, role: string }[] }) {
  ...
  // Dynamic Pricing Calculation Lookup Matrix (Images 2 & 3 Specs)
  ...
  // Compute Base Monthly Rates according to Monitoring 12h vs 24h tables
  ...
  let months = 1
  let discountPct = 0
  if (billingType === 'Quarterly') { months = 3; discountPct = 20 }
  else if (billingType === 'Half Yearly') { months = 6; discountPct = 40 }
  else if (billingType === 'Yearly') { months = 12; discountPct = 60 }
  else if (billingType === 'FOC') { months = 12; discountPct = 100 }

  const subtotalBeforeDiscount = baseMonthlyRate * months
  const priceAfterDiscount = subtotalBeforeDiscount * (1 - discountPct / 100)
  const salesTax = priceAfterDiscount * 0.05
  const grandTotal = priceAfterDiscount + salesTax
  ...
  }, [systemSizeKw, packageTier, billingType, monitoringTime, form, priceAfterDiscount, salesTax, grandTotal])
  ...
  async function onSubmit(values: z.infer<typeof customerSchema>) {
    ...
    formData.append('appliedDiscount', (subtotalBeforeDiscount * (discountPct / 100)).toString())
  ...
  const tabs = [
    ...
    { id: 3, label: 'Solar System Details' },
    { id: 4, label: 'Installer Details' },
  ]
  ...
  return (
    <div className="space-y-6">
      {/* 4 Horizontal Tabs matching EnergyGurus brand theme */}
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 bg-amber-50/50 p-2 rounded-2xl border border-amber-200/60 shadow-xs">
        {tabs.map((tab) => {
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <CardContent className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-xs font-semibold">Customer Status</FormLabel>
                        <Select onValueChange={field.onChange} value={field.value}>
                          <FormControl><SelectTrigger className="h-10 text-xs"><SelectValue /></SelectTrigger></FormControl>
                          <SelectContent>
                            <SelectItem value="Active">Active</SelectItem>
                            <SelectItem value="Pending">Pending Approval</SelectItem>
                            <SelectItem value="Temporary Blocked">Temporary Blocked</SelectItem>
                          </SelectContent>
                        </Select>
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-xs font-semibold">Activation Date</FormLabel>
                        <FormControl><Input type="date" {...field} className="h-10 text-xs" /></FormControl>
                      </FormItem>
                    )}
                  />
          ...
          {activeTab === 2 && (
            <Card className="shadow-sm border-line bg-white animate-reveal">
              <CardContent className="p-6 space-y-6">
                <div className="border-b border-line pb-3">
                  <h2 className="text-lg font-bold text-[var(--color-graphite)]">2. Package Details</h2>
                      ...
                      <FormItem>
                        ...
                        <Select onValueChange={field.onChange} value={field.value}>
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="Quarterly">Quarterly (20% Off)</SelectItem>
                            <SelectItem value="Half Yearly">Half Yearly (40% Off)</SelectItem>
                            <SelectItem value="Yearly">Yearly (60% Off)</SelectItem>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="md:col-span-2">
                        ...
                        <FormControl><Input type="number" placeholder="Enter amount paid" {...field} className="h-10 text-xs font-mono font-bold text-amber-800" /></FormControl>
                      </FormItem>
                    )}
                  />
                </div>

                {/* Calculation Summary Card based on Official Matrix */}
                <div className="bg-amber-50/50 p-5 rounded-xl border border-amber-200/80 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-xs">
                    ...
                    <div>
                      <span className="text-gray-500 block">Total Package Amount</span>
                      <span className="font-mono font-bold text-[var(--color-ink)] text-sm">PKR {Math.round(grandTotal).toLocaleString()}</span>
                    </div>
                  </div>
                </div>
          ...
          {activeTab === 3 && (
            <Card className="shadow-sm border-line bg-white animate-reveal">
              <CardContent className="p-6 space-y-6">
                <div className="border-b border-line pb-3">
                  <h2 className="text-lg font-bold text-[var(--color-graphite)]">3. Solar System Details</h2>
                ...
                <div className="flex justify-between pt-4 border-t border-line">
                  <Button type="button" variant="outline" onClick={() => setActiveTab(2)} className="text-xs gap-2">
                    <ChevronLeft className="w-4 h-4" /> Previous: Package Details
                  </Button>
                  <Button type="button" onClick={() => setActiveTab(4)} className="bg-[var(--color-amber)] hover:bg-[#d69333] text-white font-bold text-xs gap-2 px-6 shadow-sm">
          ...
          {/* TAB 4: Installer Details & Audit Checklist */}
          {activeTab === 4 && (
            <Card className="shadow-sm border-line bg-white animate-reveal">
              <CardContent className="p-6 space-y-6">
                <div className="border-b border-line pb-3">
                  <h2 className="text-lg font-bold text-[var(--color-graphite)]">4. Installer Details & System Audit</h2>
                ...
                <div className="flex justify-between items-center pt-6 border-t border-line">
                  <Button type="button" variant="outline" onClick={() => setActiveTab(3)} className="text-xs gap-2">
