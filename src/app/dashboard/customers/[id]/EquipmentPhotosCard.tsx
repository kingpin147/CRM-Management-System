    ...
    <Card className="shadow-sm border-slate-200 overflow-hidden bg-white">
      ...
      <CardContent className="p-4 sm:p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Inverter Cards */}
          {Array.from({ length: solarSystem?.noOfInverters || 1 }).map((_, i) => {
            const inverterImage = solarSystem?.inverterImages?.[i];
            const inverterSerial = solarSystem?.inverterSerials?.[i] || (i === 0 ? solarSystem?.inverterSerial : '');

            return (
              <div key={`inverter-${i}`} className="rounded-xl border border-amber-200/80 bg-gradient-to-b from-amber-50/40 to-slate-50/50 p-4 space-y-3 shadow-2xs relative">
                <div className="flex items-center justify-between border-b border-amber-100 pb-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-[#002868]">Inverter Unit {i + 1} Photo</h4>
                ...
                {inverterImage ? (
                  <div
                    onClick={() => setActiveImage({ url: inverterImage, title: `${solarSystem?.inverterBrand || 'Inverter'} ${i + 1} Photo` })}
                ...
                <div className="grid grid-cols-2 gap-2 text-xs pt-1 border-t border-slate-200/60 font-mono">
                  <div className="bg-white/80 p-2 rounded border border-slate-200/60">
                    ...
                    <span className="font-bold text-[#002868]">{inverterSerial || '—'}</span>
                  </div>
                  <div className="bg-white/80 p-2 rounded border border-slate-200/60">
          ...
          {Array.from({ length: solarSystem?.noOfBatteries || 0 }).map((_, i) => {
            const batteryImage = solarSystem?.batteryImages?.[i];
            const batterySerial = solarSystem?.batterySerials?.[i] || (i === 0 ? solarSystem?.batterySerial : '');

            return (
              <div key={`battery-${i}`} className="rounded-xl border border-slate-200 bg-gradient-to-b from-slate-50/80 to-slate-100/50 p-4 space-y-3 shadow-2xs relative">
                <div className="flex items-center justify-between border-b border-slate-200 pb-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-[#002868]">Battery Unit {i + 1} Photo</h4>
                      <p className="text-[11px] text-slate-500 font-mono">
                ...
                {batteryImage ? (
                  <div
                    onClick={() => setActiveImage({ url: batteryImage, title: `${solarSystem?.batteryBrand || 'Battery'} ${i + 1} Photo` })}
                ...
                <div className="grid grid-cols-2 gap-2 text-xs pt-1 border-t border-slate-200/60 font-mono">
                  <div className="bg-white/80 p-2 rounded border border-slate-200/60">
                    <span className="text-[10px] text-slate-500 block">Serial Number:</span>
                    <span className="font-bold text-[#002868]">{batterySerial || '—'}</span>
                  </div>
                  <div className="bg-white/80 p-2 rounded border border-slate-200/60">
                    <span className="text-[10px] text-slate-500 block">Chemistry:</span>
                    <span className="font-bold text-slate-800">
                      {solarSystem?.batteryType || 'Lithium'}
                    </span>
                  </div>
                </div>
              </div>
